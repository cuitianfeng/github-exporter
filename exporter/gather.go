...
func (e *Exporter) gatherData() ([]*Datum, error) {
	...
	for _, response := range responses {
		...
		if isArray(response.body) {
			...
			d := new(Datum)
      /*
			// Get releases from https://api.github.com/repos/<repo>/releases
			if strings.Contains(response.url, "/repos/") {
				getReleases(e, response.url, &d.Releases)
			}
			*/
			// Get latest release from https://api.github.com/repos/<repo>/releases/latest
			if strings.Contains(response.url, "/repos/") {
				getLatestRelease(e, response.url, &d.LatestRelease)
			}
...
func getLatestRelease(e *Exporter, url string, data *Release) {
	i := strings.Index(url, "?")
	baseURL := url[:i]
	releasesURL := baseURL + "/releases/latest"
	releasesResponse, err := asyncHTTPGets([]string{releasesURL}, e.APIToken())

	if err != nil {
		log.Errorf("Unable to obtain latest release from API, Error: %s", err)
	}
